...
const COMMUNITY = {
  ...
  slug: "frontend-devs",
  description:
    "A community for frontend developers to share tips, tricks, and best practices.",
...
const GROWTH_DATA = [
  ...
  { month: "Apr", members: 690 },
  { month: "May", members: 880 },
  { month: "Jun", members: 1050 },
    ...
    <div className="flex flex-col gap-6">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {p.email} · Requested {p.requestedDate}
                    </p>
                  </div>
                </div>
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      className={`rounded-full text-[10px] px-1.5 py-0 h-4 font-medium ${roleBadge(m.role)}`}
                    >
                      {m.role}
                    </Badge>
                    {m.status === "blocked" && (
              ...
              <div className="flex items-center gap-1.5 shrink-0 ml-12 sm:ml-0">
                <Select
                  value={m.role}
                  onValueChange={(v) =>
                    handleRoleChange(m.id, v as Member["role"])
                  }
                >
                  <SelectTrigger className="text-[11px] rounded-full h-7 px-2.5 w-auto gap-0.5">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button
                  ...
                  onClick={() => handleBlockToggle(m.id)}
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    setConfirmDialog({ type: "remove", id: m.id, name: m.name })
                  }
                >
                  Remove
                </Button>
      ...
      <Card className="p-5">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          />
          <Button size="sm" className="rounded-full" onClick={handleInvite}>
            Send Invite
          </Button>
        </div>
        <div className="flex items-center gap-2 mb-3">
          <Input
            value={`https://hive.ng/c/${communitySlug}/join`}
            readOnly
            className="rounded-full text-xs bg-muted/40 flex-1"
          />
          <Button
            size="icon"
            variant="outline"
            ...
            onClick={() =>
              navigator.clipboard.writeText(
                `https://hive.ng/c/${communitySlug}/join`,
              )
            }
          >
          ...
          <div className="flex flex-col gap-2">
            ...
            {invites.map((inv) => (
              ...
              >
                ...
                <Badge
                  variant="secondary"
                  className={`rounded-full text-[10px] px-1.5 py-0 h-4 ${inv.status === "accepted" ? "bg-emerald-100 text-emerald-700" : inv.status === "expired" ? "bg-muted text-muted-foreground" : ""}`}
                >
                  {inv.status}
                </Badge>
              </div>
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Remove {confirmDialog?.name} from this community? They can be
              re-invited later.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2">
            ...
            <Button
              variant="destructive"
              className="rounded-full"
              onClick={() => confirmDialog && handleRemove(confirmDialog.id)}
            >
              Remove
            </Button>
          </DialogFooter>
...
function SettingsTab({
  ...
  community: NonNullable<ReturnType<typeof resolveCommunity>>;
}) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <Card className="p-5 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center rounded-xl border bg-muted/40 px-3 py-2 gap-1.5 text-sm">
            <span className="text-muted-foreground shrink-0">hive.ng/c/</span>
            <input
              value={form.slug}
        ...
        <div className="flex flex-wrap gap-2">
          {VISIBILITY_OPTIONS.map((opt) => (
            <button
              ...
              className={`flex-1 min-w-[140px] rounded-xl border-2 px-4 py-3 text-left transition-colors ${form.visibility === opt.value ? "border-foreground bg-muted/40" : "border-border hover:border-muted-foreground/30"}`}
            >
            ...
            <div className="flex flex-col gap-1.5 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="relative">
                ...
                <Input
                  value={form.price}
                  onChange={(e) =>
                    update("price", formatPriceInput(e.target.value))
                  }
                  placeholder="2,500"
      ...
      <Card className="p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium">Sequential Courses</p>
            <p className="text-xs text-muted-foreground">
              Courses must be completed in order
            </p>
          </div>
          <Switch
            checked={form.sequentialCourses}
            onCheckedChange={(v) => update("sequentialCourses", v)}
          />
        </div>
        <Separator />
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Members can download course materials
            </p>
          </div>
          <Switch
            checked={form.allowDownloads}
            onCheckedChange={(v) => update("allowDownloads", v)}
          />
        </div>
        <Separator />
        <div className="flex flex-col gap-1.5">
          <Label className="text-xs font-medium">Max Concurrent Devices</Label>
          <Input
            type="number"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-[10px] text-muted-foreground">
            Days before late payment triggers suspension
          </p>
        </div>
      </Card>
      ...
      <Card className="p-5 border-destructive/30">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Archiving will hide this community from non-members. Existing members
          will retain access.
        </p>
        <Button
          variant="destructive"
      ...
      <Dialog open={archiveOpen} onOpenChange={setArchiveOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Archive Community</DialogTitle>
            <DialogDescription>
              This will hide "{form.name}" from non-members. Existing members
              keep access. This can be reversed from settings.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2">
            <Button
...
function CommunityCoursesTab({
  ...
  const communityCourses = COURSES_DATA.filter(
    (c) => c.communitySlug === communitySlug,
  );
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          render={
            <Link
              href={`/dashboard/courses/create?role=${role}&community=${encodeURIComponent(communityName)}`}
            >
      ...
      {communityCourses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {communityCourses.map((c) => (
            <Link
              key={c.slug}
              href={`/dashboard/courses/${c.slug}/manage?role=${role}`}
            >
              <Card className="p-4 hover:bg-muted/40 transition-colors flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Badge
                      className={`rounded-full text-[10px] px-2 py-0 h-5 ${diffColors2[c.difficulty]}`}
                    >
                      {c.difficulty}
                    </Badge>
...
const MOCK_POSTS: Post[] = [
  {
    ...
    role: "Owner",
    content:
      "Welcome to Frontend Devs! 🚀 This is our community space for sharing tips, asking questions, and showcasing your work. Please read the pinned guidelines before posting.",
    type: "announcement",
    likes: 24,
    liked: false,
    comments: [
      {
        author: "Chioma Nwosu",
        initials: "CN",
        text: "Excited to be here! Looking forward to learning from everyone.",
        time: "1h ago",
      },
    ],
    time: "3d ago",
  ...
  },
  {
    ...
    role: "Member",
    content:
      "Just shipped a new React component library! It includes accessible form controls, a flexible grid system, and dark mode support out of the box. Would love feedback from the community.\n\nCheck it out: github.com/kelechi/react-ui",
    type: "post",
    likes: 18,
    liked: true,
    comments: [],
  ...
  },
  {
    ...
    role: "Member",
    content:
      "Has anyone worked with the new View Transitions API? I'm building a page transition system and it feels like magic. Would love to compare notes.",
    type: "post",
    likes: 7,
    liked: false,
    comments: [
      {
        ...
        initials: "TB",
        text: "Yes! Used it on my portfolio. The cross-fade between pages is buttery smooth.",
        time: "30m ago",
      },
    ],
    time: "5h ago",
  ...
  },
  {
    ...
    role: "Owner",
    content:
      "📢 Community update: We've reached 1,200 members! To celebrate, I'm hosting a live code review session this Friday at 6pm WAT. Bring your projects — I'll review as many as I can in 90 minutes.\n\nThe session will be recorded for those who can't make it live.",
    type: "announcement",
    likes: 42,
    liked: false,
    comments: [
...
function FeedTab() {
  ...
  const toggleComments = (id: string) =>
    setPosts((p) =>
      p.map((x) => (x.id === id ? { ...x, showComments: !x.showComments } : x)),
    );
  ...
  const saveEdit = () => {
    ...
    setPosts((p) =>
      p.map((x) => (x.id === editId ? { ...x, content: editText } : x)),
    );
    setEditId(null);
  };
    ...
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      {!composeOpen ? (
        <button
          ...
          className="w-full rounded-xl border-2 border-dashed border-border hover:border-muted-foreground/30 p-4 flex items-center gap-3 bg-muted/20 text-left transition-colors"
        >
        ...
        <Card className="p-4 flex flex-col gap-3 animate-in fade-in">
          <div className="flex items-center gap-2">
            {(
              [
                { v: "post" as const, l: "Post" },
                { v: "announcement" as const, l: "Announcement" },
              ] as const
            ).map((o) => (
              <button
                key={o.v}
                onClick={() => setComposeType(o.v)}
                className={`text-xs px-3 py-1.5 rounded-full border transition-colors flex items-center gap-1 ${composeType === o.v ? "border-foreground bg-muted font-medium" : "border-border hover:border-muted-foreground/30"}`}
              >
                <HugeiconsIcon
                  icon={
                    o.v === "announcement" ? Megaphone01Icon : Comment01Icon
                  }
                  size={12}
                />
                {o.l}
      ...
      {sorted.map((post) => (
        <Card
          key={post.id}
          className={`p-5 ${post.type === "announcement" ? "border-primary/30 bg-primary/5 ring-1 ring-primary/20" : ""}`}
        >
          {/* Header */}
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge className="rounded-full text-[9px] px-1.5 py-0 h-4">
                    {post.role}
                  </Badge>
                  {post.type === "announcement" && (
                    <Badge className="rounded-full text-[9px] px-1.5 py-0 h-4 bg-amber-100 text-amber-700">
                      <HugeiconsIcon
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="size-7 flex items-center justify-center rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground"
              >
                <HugeiconsIcon
                  icon={PinIcon}
                  size={14}
            ...
            <div className="flex flex-col gap-2 mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className="rounded-full h-7 text-xs text-destructive"
                  onClick={() => {
                    deletePost(post.id);
                    setEditId(null);
                  }}
          ...
          <div className="flex items-center gap-1 border-t pt-3">
            <button
              ...
              className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs transition-colors ${post.liked ? "bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400" : "text-muted-foreground hover:bg-muted"}`}
            >
              <HugeiconsIcon
                icon={HeartIcon}
                size={14}
                className={post.liked ? "fill-rose-500" : ""}
              />
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted transition-colors"
            >
      ...
      <DashboardLayout role={role}>
        <div className="flex flex-col items-center justify-center py-24 text-center gap-4 min-w-0">
          ...
          <Button
            ...
            className="rounded-full"
            render={
              <Link href={`/dashboard/communities?role=${role}`}>
                Back to Communities
              </Link>
            }
          />
      ...
      <div className="flex flex-col gap-6 min-w-0">
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <Badge variant="secondary" className="rounded-full w-fit">
              <HugeiconsIcon icon={UserGroupIcon} size={13} className="mr-1" />
          ...
          <div className="flex items-center gap-1 w-max">
            {TABS.map(({ key, label, icon }) => (
              <button
                ...
                type="button"
                onClick={() => setTab(key)}
                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full transition-colors whitespace-nowrap ${tab === key ? "bg-foreground text-background font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
              >
        ...
        {tab === "overview" && (
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: "Members", value: "1,248", icon: UserGroupIcon },
                { label: "Active (7d)", value: "342", icon: ChartBarLineIcon },
                { label: "Revenue (mo)", value: "₦156,000", icon: Add01Icon },
                { label: "Courses", value: "8", icon: LayoutGridIcon },
              ].map((s) => (
                <Card key={s.label} className="p-4">
                  <HugeiconsIcon
                    icon={s.icon}
