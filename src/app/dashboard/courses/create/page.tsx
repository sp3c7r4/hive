...
const DIFFICULTY_OPTIONS: { value: Difficulty; label: string; desc: string }[] = [
  ...
  { value: "intermediate", label: "Intermediate", desc: "Some experience recommended" },
      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-6 min-w-0">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {community ? `Adding to ${community}` : "Build your course curriculum and set it live"}
          </p>
        </div>

        {/* ---- Cover Image ---- */}
        <Card className="p-5">
          <Label className="text-sm font-semibold mb-3 block">Cover Image</Label>
          {coverPreview ? (
            <div className="relative rounded-xl overflow-hidden aspect-[2.5/1] bg-muted group">
              ...
              <button type="button" onClick={() => { URL.revokeObjectURL(coverPreview); setCoverPreview(null); }}
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              ...
              <div className="size-12 rounded-full bg-muted flex items-center justify-center"><HugeiconsIcon icon={Image01Icon} size={22} className="text-muted-foreground" /></div>
              <p className="text-sm font-medium">Upload a cover image</p><p className="text-xs text-muted-foreground">16:9 recommended</p>
            </button>
          )}
        ...
        <Card className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5"><Label className="text-xs font-medium">Title</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder='e.g. "React for Designers"' className="rounded-xl" maxLength={120} /></div>
          <div className="flex flex-col gap-1.5"><Label className="text-xs font-medium">Description</Label><Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What will students learn? Who is this course for?" className="rounded-xl min-h-[100px] resize-y" maxLength={800} />
        ...
        <Card className="p-5 flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between"><div><p className="text-sm font-medium">This course is free</p><p className="text-xs text-muted-foreground">Anyone can enroll at no cost</p></div><Switch checked={isFree} onCheckedChange={setIsFree} /></div>
          {!isFree && (
            <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-top-2">
              <Separator />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <Label className="text-xs font-medium">One-time Price (₦)</Label>
                  <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground font-medium">₦</span>
                    <Input value={oneTimePrice} onChange={(e) => setOneTimePrice(fmtPrice(e.target.value))} placeholder="5,000" className="rounded-xl pl-8" />
                  </div>
                  <p className="text-[10px] text-muted-foreground">Leave empty for no one-time option</p>
                </div>
                <div className="flex flex-col gap-1.5">
                  <Label className="text-xs font-medium">Monthly Subscription (₦)</Label>
                  <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground font-medium">₦</span>
                    <Input value={monthlyPrice} onChange={(e) => setMonthlyPrice(fmtPrice(e.target.value))} placeholder="2,000" className="rounded-xl pl-8" />
                  </div>
                  <p className="text-[10px] text-muted-foreground">Leave empty for no subscription option</p>
                </div>
              </div>
            </div>
          )}
        ...
        <Card className="p-5 flex flex-col gap-4">
          ...
          {[
            { label: "Sequential Access", desc: "Students must complete lessons in order", state: sequentialAccess, set: setSequentialAccess },
            { label: "Drip Content", desc: "Release lessons on a schedule instead of all at once", state: dripContent, set: setDripContent },
            { label: "Allow Comments", desc: "Students can comment on lessons", state: allowComments, set: setAllowComments },
            { label: "Allow Downloads", desc: "Students can download course materials", state: allowDownloads, set: setAllowDownloads },
          ].map((item, i) => (
            <div key={item.label}>
        ...
        <Card className="p-5 flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between"><div><p className="text-sm font-medium">Offer Certificate</p><p className="text-xs text-muted-foreground">Students earn a certificate upon completion</p></div><Switch checked={offerCertificate} onCheckedChange={setOfferCertificate} /></div>
          {offerCertificate && (
            <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-top-2">
              <Separator />
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  { label: "Min Completion %", value: minCompletion, set: setMinCompletion },
                  { label: "Min Quiz Score %", value: minQuizScore, set: setMinQuizScore },
                  { label: "Min Attendance %", value: minAttendance, set: setMinAttendance },
                ].map((f) => (
                  <div key={f.label} className="flex flex-col gap-1.5">
                    <Label className="text-xs font-medium">{f.label}</Label>
                    <Input type="number" min={0} max={100} value={f.value} onChange={(e) => f.set(e.target.value)} className="rounded-xl" />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 justify-end">
          <Button variant="outline" className="rounded-full" onClick={() => router.back()}>Cancel</Button>
          <Button variant="outline" className="rounded-full" disabled={!isValid}>Save Draft</Button>
          <Button className="rounded-full" disabled={!isValid} onClick={() => setPublishOpen(true)}>Publish</Button>
        </div>

        <Dialog open={publishOpen} onOpenChange={setPublishOpen}>
          <DialogContent className="max-w-sm">
            <DialogHeader><DialogTitle>Publish Course</DialogTitle><DialogDescription>This will make the course visible to community members. You can still edit it after publishing.</DialogDescription></DialogHeader>
            <DialogFooter className="gap-2"><Button variant="outline" className="rounded-full" onClick={() => setPublishOpen(false)}>Cancel</Button><Button className="rounded-full" onClick={handlePublish}>Confirm & Publish</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
    </DashboardLayout>
...
export default function Wrapper() {
  return <Suspense fallback={null}><CreateCoursePage /></Suspense>;
}
