...
function MembersPage() {
  ...
  const filtered = members.filter((m) => {
    if (
      ...
      return false;
    if (
      communityFilter !== "All Communities" &&
      m.community !== communityFilter
    )
      return false;
      ...
      <div className="flex flex-col gap-5 min-w-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage members across your communities
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card className="p-4">
            ...
            <p className="text-xl font-bold tabular-nums mt-1 text-emerald-600">
              {totalActive}
            </p>
          </Card>
          <Card className="p-4">
            <p className="text-[10px] text-muted-foreground">Pending</p>
            <p className="text-xl font-bold tabular-nums mt-1 text-amber-600">
              {totalPending}
            </p>
          </Card>
          <Card className="p-4">
            <p className="text-[10px] text-muted-foreground">Blocked</p>
            <p className="text-xl font-bold tabular-nums mt-1 text-rose-600">
              {totalBlocked}
            </p>
          </Card>
        </div>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {m.status === "pending" && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="rounded-full h-7 text-[10px] text-emerald-600"
                        ...
                        {m.status === "active" && (
                          <Button
                            ...
                            onClick={() =>
                              setConfirmAction({ id: m.id, action: "block" })
                            }
                          >
                            Block
                          </Button>
                        ...
                        {m.status === "blocked" && (
                          <Button
                            ...
                            variant="ghost"
                            className="rounded-full h-7 text-[10px] text-emerald-600"
                            onClick={() => handleAction(m.id, "unblock")}
                          >
                            Unblock
                          </Button>
                        )}
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-1 mb-5">
              Are you sure you want to {confirmAction.action} this member?
            </p>
            <div className="flex justify-end gap-2">
              ...
              <Button
                ...
                className="rounded-full"
                variant={
                  confirmAction.action === "block" ? "destructive" : "default"
                }
                onClick={() =>
